<template>
  <v-card variant="outlined" class="mb-4 pa-4 pt-0">
    <v-card-title>Filter Matches</v-card-title>
    <v-card-text>
      Use the filters below to find matches. Select a match type, players, or teams to narrow down the results.
    </v-card-text>
    <v-row>
      <v-col cols="12" md="4">
        <v-select
...
export default defineComponent({
  ...
  setup() {
    ...
    const matchTypes = ['individual', 'team'];
    const selectedMatchType = ref<string | null>(null);
    const selectedBlue = ref<string | null>(null);
    const selectedRed = ref<string | null>(null);
    ...
    const filteredMatches = computed(() => {
      ...
      .filter((match) => {
        const blueMatches =
          !selectedBlue.value ||
          match.opponents.blue.includes(selectedBlue.value) || 
          match.opponents.red.includes(selectedBlue.value);
        const redMatches =
          !selectedRed.value ||
          match.opponents.blue.includes(selectedRed.value) || 
          match.opponents.red.includes(selectedRed.value);
